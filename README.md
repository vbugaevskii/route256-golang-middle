...
1. [x] Если пользователь уже запрашивал историю за конкретный период, то возвращать её из кэша
2. [x] Для кэширования использовать либо LRU с воркшопа, либо любое другое решение, например, Redis
3. [ ] Функционал покрыть unit тестами
4. [x] Требуется обеспечить простоту замены конкретной реализации кэша
5. [x] 💎 На алмазик реализовать кэширование (с практической пользой) в другом сервисе/участке системы на свое усмотрение
